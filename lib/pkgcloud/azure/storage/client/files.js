...
exports.removeFile = function (container, file, callback) {
  ...
  this._request(
    ...
    function (err, body, res) {
      return err ? callback(err) : callback(null, res.statusCode == 202);
    }
  );
};
...
exports.multipartUpload = function (options) {
  var self = this,
    numberOfBlocks = 0,
    chunkedStream = new storage.ChunkedStream(
      AzureConstants.BlobConstants.DEFAULT_WRITE_BLOCK_SIZE_IN_BYTES
    ),
    proxyStream = through(),
    ended = false;
      ...
      if (err) {
        ...
        self.sendBlockList(
          _.extend(
            ...
            _.pick(options, ['remote', 'container', 'headers', 'contentType'])
          ),
          function (err) {
            if (err) {
              proxyStream.emit('error', err);
...
exports.sendBlockList = function (options, callback) {
  ...
  if (options.contentType) {
    ...
    putOptions.headers['x-ms-blob-content-type'] = mime.getType(options.remote);
  }
  ...
  this._xmlRequest(
    ...
    function (err, body) {
      ...
      if (body.Blobs && body.Blobs.Blob) {
        return callback(
          null,
          self._toArray(body.Blobs.Blob).map(function (file) {
