...
Client.prototype.getUserAgent = function () {
  return util.format(
    '%snodejs-pkgcloud/%s',
    this._customUserAgent ? this._customUserAgent + ' ' : '',
    pkgcloud.version
  );
};
...
Client.prototype._request = function (options, callback) {
  ...
  requestOptions.strictSSL =
    typeof self.config.strictSSL === 'boolean' ? self.config.strictSSL : true;
...
Client.prototype._parseError = function (response, body) {
  ...
  if (Object.keys(self.failCodes).indexOf(statusCode) !== -1) {
    ...
    err = {
      ...
      message: self.provider + ' Error (' + statusCode + '): ' + self.failCodes[statusCode],
